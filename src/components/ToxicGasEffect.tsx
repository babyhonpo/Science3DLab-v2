...
export default function ToxicGasEffect({ position }: ToxicGasEffectProps) {
  ...
  useFrame((delta) => {
    ...
    for (let i = 0; i < count; i++) {
      // 速度に基づいて位置を更新
      newPositions[i * 3] =
        positionArray[i * 3] + particleProps.velocities[i * 3];
      newPositions[i * 3 + 1] =
        positionArray[i * 3 + 1] + particleProps.velocities[i * 3 + 1];
      newPositions[i * 3 + 2] =
        positionArray[i * 3 + 2] + particleProps.velocities[i * 3 + 2];
  ...
  const particleGeometry = useMemo(() => {
    ...
    geometry.setAttribute(
      "position",
      new THREE.Float32BufferAttribute(particleProps.positions, 3)
    );
    geometry.setAttribute(
      "size",
      new THREE.Float32BufferAttribute(particleProps.sizes, 1)
    );
    geometry.setAttribute(
      "opacity",
      new THREE.Float32BufferAttribute(particleProps.opacities, 1)
    );
    return geometry;

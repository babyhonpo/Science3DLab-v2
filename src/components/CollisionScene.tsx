...
function CollisionObjects() {
  ...
  const [collisionPoint, setCollisionPoint] = useState<THREE.Vector3 | null>(
    null
  );
  ...
  useFrame((state) => {
    if (sphere1.current && sphere2.current) {
      ...
      sphere2.current.position.x =
        Math.sin(state.clock.elapsedTime * 2 + Math.PI) * 1.5;

      // Check for collision
      const distance = sphere1.current.position.distanceTo(
